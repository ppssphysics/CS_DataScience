...
# paths <- c("Data/en_US/en_US.blogs.txt","../Data/en_US/en_US.news.txt","../Data/en_US/en_US.twitter.txt")
...
for (i in 1:length(listngrams)){
	for (j in 1:nrow(listngrams[[i]])){
		...
		listngrams[[i]]$NextWord[j] <- getlastwords(as.character(listngrams[[i]]$`Unique Words`[j]),1)

	}
}

dn2 <- subset(dn2,Frequency>1)
dn3 <- subset(dn3,Frequency>1)
dn4 <- subset(dn4,Frequency>1)
dn5 <- subset(dn5,Frequency>1)
